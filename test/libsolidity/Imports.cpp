...
BOOST_AUTO_TEST_CASE(inheritance_abi_encoder_mismatch_1)
{
	...
	for (auto const& e: c.errors())
	{
		...
		string const* msg = e->comment();
		BOOST_REQUIRE(msg);
		BOOST_CHECK_EQUAL(*msg, std::string("Contract \"C\" doesn't use the new experimental ABI encoder but wants to inherit from contract \"A\" which uses types that require it. Use \"pragma experimental ABIEncoderV2;\" for the inheriting contract as well to enable the feature."));
	}
...
BOOST_AUTO_TEST_CASE(inheritance_abi_encoder_mismatch_2)
{
	...
	for (auto const& e: c.errors())
	{
		...
		string const* msg = e->comment();
		BOOST_REQUIRE(msg);
		BOOST_CHECK_EQUAL(*msg, std::string("Contract \"B\" doesn't use the new experimental ABI encoder but wants to inherit from contract \"A\" which uses types that require it. Use \"pragma experimental ABIEncoderV2;\" for the inheriting contract as well to enable the feature."));
	}
	BOOST_CHECK_EQUAL(typeErrors, 1);
}
